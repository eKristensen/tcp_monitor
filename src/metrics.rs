...
pub struct Metrics {
    ...
    // --- client side ---
    pub clt_session_active: Family<PeerLabel, Gauge>,
    pub clt_session_start: Family<PeerLabel, Gauge<f64, std::sync::atomic::AtomicU64>>,
    pub clt_session_duration: Family<PeerLabel, Gauge<f64, std::sync::atomic::AtomicU64>>,
    pub clt_sessions_total: Family<PeerLabel, Counter>,
    pub clt_heartbeats_sent: Family<PeerLabel, Counter>,
    pub clt_heartbeats_rx: Family<PeerLabel, Counter>,
    pub clt_heartbeats_missed: Family<PeerLabel, Counter>,
    pub clt_consecutive_missed: Family<PeerLabel, Gauge>,
    pub clt_rtt: Family<PeerLabel, Gauge<f64, std::sync::atomic::AtomicU64>>,
    pub clt_last_heartbeat: Family<PeerLabel, Gauge<f64, std::sync::atomic::AtomicU64>>,
    pub clt_disconnects: Family<DisconnectLabel, Counter>,
...
impl Metrics {
    ...
    pub fn new() -> Arc<Self> {
        ...
        Arc::new(Metrics {
            ...
            clt_session_active: reg!("tcp_monitor_client_session_active",
                "1 if a client session is currently active to this peer"),
            clt_session_start: reg!("tcp_monitor_client_session_start_timestamp_seconds",
                "Unix timestamp when the current/last client session was established"),
            clt_session_duration: reg!("tcp_monitor_client_session_duration_seconds",
                "Duration of the active or last completed client session"),
            clt_sessions_total: reg!("tcp_monitor_client_sessions",
                "Total outbound sessions established since startup"),
            clt_heartbeats_sent: reg!("tcp_monitor_client_heartbeats_sent",
                "Total heartbeat packets sent to a peer"),
            clt_heartbeats_rx: reg!("tcp_monitor_client_heartbeats_received",
                "Total heartbeat echoes received from a peer"),
            clt_heartbeats_missed: reg!("tcp_monitor_client_heartbeats_missed",
                "Total heartbeat echoes not received within the timeout window"),
            clt_consecutive_missed: reg!("tcp_monitor_client_heartbeats_consecutive_missed",
                "Current run of consecutive unanswered heartbeats"),
            clt_rtt: reg!("tcp_monitor_client_heartbeat_rtt_seconds",
                "Round-trip time of the most recent heartbeat echo"),
            clt_last_heartbeat: reg!("tcp_monitor_client_last_heartbeat_timestamp_seconds",
                "Unix timestamp of the most recent successful heartbeat echo"),
            clt_disconnects: reg!("tcp_monitor_client_session_disconnects",
    ...
    pub fn init_disconnect_labels(&self, node: &str, peer: &str) {
        ...
        for reason in &["remote_close", "connection_reset", "timeout", "local_error", "connect_failed"] {
            ...
            let _ = self.clt_disconnects.get_or_create(&label);
        }
    }
}

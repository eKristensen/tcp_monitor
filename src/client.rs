    ...
    loop {
        ...
        match connect_result {
            Err(e) => {
                ...
                metrics.cli_disconnects.get_or_create(&label).inc();
...
async fn run_session(
    ...
    metrics.cli_session_active.get_or_create(&label).set(1);
    metrics.cli_session_start.get_or_create(&label).set(start_ts);
    metrics.cli_sessions_total.get_or_create(&label).inc();
    metrics.cli_consecutive_missed.get_or_create(&label).set(0);
    ...
    let reason: &'static str = 'session: loop {
        ...
        metrics.cli_heartbeats_sent.get_or_create(&label).inc();
        seq += 1;
        ...
        match echo_result {
            Err(_timeout) => {
                ...
                metrics.cli_heartbeats_missed.get_or_create(&label).inc();
                metrics.cli_consecutive_missed.get_or_create(&label).set(consecutive_missed);
                warn!(
            ...
            Ok(Ok(_echo)) => {
                ...
                let now_ts = now_f64();
                metrics.cli_heartbeats_rx.get_or_create(&label).inc();
                metrics.cli_consecutive_missed.get_or_create(&label).set(0);
                metrics.cli_rtt.get_or_create(&label).set(rtt);
                metrics.cli_last_heartbeat.get_or_create(&label).set(now_ts);
                metrics.cli_session_duration.get_or_create(&label).set(now_ts - start_ts);
            }
        }
    };

    let duration = now_f64() - start_ts;
    metrics.cli_session_active.get_or_create(&label).set(0);
    metrics.cli_session_duration.get_or_create(&label).set(duration);

    if reason != "cancelled" {
        metrics.cli_disconnects.get_or_create(&DisconnectLabel {
